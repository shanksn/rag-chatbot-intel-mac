...
class CourseSearchTool(Tool):
    ...
    def _format_results(self, results: SearchResults) -> str:
        ...
        for doc, meta in zip(results.documents, results.metadata):
            ...
            source_key = f"{course_title}|{lesson_num}|{course_link}"
...
class CourseOutlineTool(Tool):
    """Tool for getting course outlines and metadata"""
    ...
    def get_tool_definition(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "description": "Get course title, link, instructor, and complete lesson list for a course",
            "input_schema": {
                "type": "object",
                "properties": {
                    "course_name": {
    ...
    def execute(self, course_name: str) -> str:
        """
        Get course outline and metadata.
        ...
        # Use the vector store's course catalog search to find the course
        results = self.store.search_course_catalog(course_name, max_results=1)
        ...
        course_instructor = metadata.get('course_instructor', '')
        ...
        if course_instructor:
            outline.append(f"**Course Instructor:** {course_instructor}")
        
        if lessons:
            outline.append(f"**Number of Lessons:** {len(lessons)}")
            outline.append("")
            outline.append("**Complete Lesson List:**")
            
            for i, lesson in enumerate(lessons):
                lesson_title = lesson.get('title', f'Lesson {i}')
                lesson_link = lesson.get('link', '')
                
                lesson_line = f"{i}: {lesson_title}"
                if lesson_link:
                    lesson_line += f" ({lesson_link})"
                
                outline.append(lesson_line)
        else:
            outline.append("**Lessons:** No lesson information available")
